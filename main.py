...
# Mobile Window Preview - sirf desktop testing ke liye, Android par apply nahi hoga
...
class MusicPlayer(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # Header Title
        self.add_widget(Label(text='[b]My Music Streamer[/b]', markup=True, font_size='22sp', size_hint_y=0.1))

        # Search Bar Section
        search_box = BoxLayout(orientation='horizontal', size_hint_y=0.1, spacing=5)
        self.search_input = TextInput(hint_text='Song name enter karein...', multiline=False)
        ...
        # Status & Track Info
        self.status = Label(text='Song search karein ya stream play karein', size_hint_y=0.1, font_size='14sp')
        self.add_widget(self.status)

        # Direct Stream Player Controls
        controls = BoxLayout(orientation='horizontal', size_hint_y=0.15, spacing=10)
        self.play_btn = Button(text='PLAY DEMO', background_color=(0.2, 0.8, 0.4, 1))
        self.play_btn.bind(on_press=self.play_demo)

        self.stop_btn = Button(text='STOP', background_color=(0.9, 0.2, 0.2, 1))
    ...
    def search_song(self, instance):
        ...
        if not query:
            self.status.text = "Kripya gaane ka naam likhein!"
            return

        self.status.text = f"Searching for: {query}..."

        # JioSaavn Unofficial API (Educational Use)
        encoded_query = urllib.parse.quote(query)
        api_url = f"https://saavn.dev/api/search/songs?query={encoded_query}&limit=1"

        UrlRequest(api_url, on_success=self.on_search_success, on_failure=self.on_search_error, on_error=self.on_search_error)
    ...
    def on_search_success(self, request, result):
        try:
            ...
            if songs:
                ...
                # High quality stream url fetch karna
                download_urls = song.get('downloadUrl', [])
                if download_urls:
                    self.current_stream_url = download_urls[-1].get('url')  # Max quality link
                    self.status.text = f"Found: {song_name}\nReady to play!"
                else:
                    self.status.text = "Stream URL nahi mili."
            else:
                self.status.text = "Koi gaana nahi mila."
        except Exception as e:
            self.status.text = "Error parsing response."

    def on_search_error(self, request, error):
        self.status.text = "Network Error! Check Internet Connection."
    ...
    def play_demo(self, instance):
        ...
        if self.sound:
            ...
            self.status.text = "Audio stream load nahi ho saka."
